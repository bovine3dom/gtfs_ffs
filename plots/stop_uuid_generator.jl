...
select_df(con(), """select count() c from transitous_everything_stops""")
df = select_df(con(), """
   select groupArray(h3) overlaps from (
       -- 2 is too aggressive... 1 works kinda
        select distinct geoToH3(stop_lat, stop_lon, 11) h3, arrayJoin(h3kRing(h3, 1)) h3_ring from transitous_everything_stops
   )
   group by h3_ring
""")
...
execute(con(), "drop table if exists tmp_transitous_everything_stop_uuids")
execute(con(), """
CREATE TABLE tmp_transitous_everything_stop_uuids
(
    `h3` UInt64,
    `stop_uuid` Int64
)
...
""")
insert(con(), "tmp_transitous_everything_stop_uuids",[df2dict(agg)])
