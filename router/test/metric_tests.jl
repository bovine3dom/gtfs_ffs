...
@testset "Selectable time/distance quantile metric" begin
    ...
    response = make_handler(capped)(HTTP.Request("GET", "/reachable?$index&$times&window_h=0.016944444444444446&step_h=0.016666666666666666&$metric"))
    table = Arrow.Table(response.body)
    @test all(iszero, table.value)
    @test sort(collect(table.elapsed_h)) == [0, 80_000] ./ 3_600_000
    @test sort(collect(table.reachable_elapsed_h)) == [0, 80_000] ./ 3_600_000
